...
uint8_t in_main_page = 1;	//��ǰ�Ƿ��������棬ˢ����ҳ�����ݱ�־λ 0���� 1����
uint8_t pre_language = 0;	//��ǰ����
uint8_t pre_main_page = 0;   //��ǰ��ҳ��ҳ����
uint16_t pre_system_sta = 0; //��ǰϵͳ״̬
uint8_t set_num = 0;		 //�����趨��������λ��
uint8_t set_pid_channel = 0;
int8_t curve_page_num = 0;   //���߽����
int8_t curve_time_level = 0; //����ʱ�����С

uint8_t output_rate[12] = {0}; //��ȡ
uint8_t sensor_sta[12] = {0};  //��ȡ
int16_t run_temp[12] = {0};	//��ȡ
uint8_t alarm_type[12] = {0};  //��ȡ

int16_t set_temp[12] = {100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100}; //����
...
uint16_t p_value[12] = {100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100}; //����
uint16_t i_value[12] = {100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100}; //����
uint16_t d_value[12] = {100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100}; //����

uint16_t preheat_time = 3;	//����
uint16_t all_sensor_type = 0; //����
...
uint8_t alarm_msg[4][16] = {{0xB8, 0xD0, 0xCE, 0xC2, 0xCF, 0xDF, 0xB6, 0xCF, 0xBF, 0xAA},									   //�����߶Ͽ�
							{0xB8, 0xD0, 0xCE, 0xC2, 0xCF, 0xDF, 0xBD, 0xD3, 0xB7, 0xB4},									   //�����߽ӷ�
							{0xB3, 0xAC, 0xCE, 0xC2, 0xB1, 0xA8, 0xBE, 0xAF},												   //���±���
							{0xC9, 0xFD, 0xCE, 0xC2, 0xB5, 0xBD, 0xB4, 0xEF, 0xC9, 0xE8, 0xB6, 0xA8, 0xCE, 0xC2, 0xB6, 0xC8}}; //���µ����趨�¶�
...
void init_time_ctrl_value(void)
{
	...
	for (i = 0; i < 4; i++)
	{
		for (j = 0; j < 8; j++)
		{
			for (k = 0; k < 4; k++)
			{
				time_ctrl_value[i][j][k] = 500; //�佺����ʱ���������ݳ�ʼ��Ϊ
...
void update_main_page(void)
{
	...
	for (i = 0; i < 6; i++)
	{
		send_variables(MAINPAGE_NUM1_ADDR + (i * 2), (i + (pre_main_page * 6) + 1));		//��ҳ�洫���������ʾ
		send_variables(MAIN_OUTRATE1_ADDR + (i * 2), output_rate[i + (pre_main_page * 6)]); //�������������
		send_variables(MAIN_RUNTEMP1_ADDR + (i * 2), run_temp[i + (pre_main_page * 6)] +
														 temp_unit * (run_temp[i + (pre_main_page * 6)] * 8 / 10 + 32)); //�����¶�
		send_variables(MAIN_SETTEMP1_ADDR + (i * 2), set_temp[i + (pre_main_page * 6)]);								 //�趨�¶�
...
void update_single_set_page(void)
{
	...
	send_variables(SINGLE_NUM_ADDR, set_num + 1);															  //�����ʾ
	send_variables(SINGLE_RUNTEMP_ADDR, run_temp[set_num] + temp_unit * ((run_temp[set_num] * 8 / 10) + 32)); //�����¶���ʾ
	send_variables(SINGLE_SETTEMP_ADDR, set_temp[set_num]);													  //�趨�¶���ʾ
	send_variables(SINGLE_SET_TEMP, set_temp[set_num]);														  //�¶��趨�������ʾ
	send_variables(SINGLE_SET_SWITCH, switch_sensor[set_num]);												  //����������/�ر�״̬
	send_variables(SINGLE_SENSORTYPE_ADDR, TYPE_J + (sensor_type[set_num] * TYPE_K));						  //DANDUPAGE����������ѡ����ʾ
...
void update_curve_page(void)
{
	...
	send_variables(CURVE_PAGE_SETTEMP_ADDR, set_temp[curve_page_num]);								   //�趨�¶�
...
void change_page(uint8_t page_num)
{
	...
	usart0_tx_buff[2] = 0x04; //4.3����Ϊ4  5����Ϊ6
